...
v1.8 是**数据研发 / 数仓 ETL 视角的能力补全**：在 v1.7 已具备 local-first runtime + Argo local DAG + multi-source QC contract 之后，v1.8 不继续堆 K8s 复杂度，而是把"运行时事件流"沉淀成可查询、可看板、可补数的**结构化指标**。

### 1. 为什么 v1.8 不继续堆 K8s

v1.5 ~ v1.7 已经把 Argo / kind / PVC / archive logs / 多源 contract / ML-ready / Go exporter 走完。继续往 K8s 方向走会得不偿失（运维面太大、与岗位面试关键词重合度低）。v1.8 把视角切到**数据研发岗常考的能力**：DIM/FACT/DWS/ADS 数仓分层、SQL build job、UPSERT 幂等、数据质量日报、Top N 失败、SLA 校验、补数 plan。

### 2. DIM / FACT / DWS / ADS 分层

15 张新表（远端 PostgreSQL 真源 = `postgres/migrations/006_v1_8_warehouse_quality_ops.sql`；本地 SQLite 由 `WarehouseBase` 同步建出，用于 `make test` / 离线 demo）：

| 层 | 表 | 来源 |
|---|---|---|
| DIM | `dim_dataset` | `dataset_versions` + `quality_snapshots` + `ml_ready_datasets` |
| FACT | `fact_etl_run` / `fact_qc_rule_result` / `fact_workflow_step` / `fact_asset_profile` | v1.5/v1.6 业务表增量物化 |
| DWS | `dws_dataset_quality_daily` / `dws_rule_failure_daily` / `dws_workflow_ops_daily` | FACT 按 (dt, dataset_id, version) / (dt, dataset_family, rule_id) / (dt, workflow_type) 聚合 |
| ADS | `ads_quality_dashboard` / `ads_workflow_ops_dashboard` | DWS 聚合 + 阈值判定 → `overall_status` / `quality_score` / `alert_level` |
| Backfill | `backfill_plans` / `backfill_tasks` | `robot-dh backfill plan` 写入 |
| SLA | `sla_policies` / `sla_checks` / `dataset_partition_readiness` | `robot-dh sla check` 写入 |

### 3. 核心 CLI

```bash
# 检查 v1.8 表是否齐全（本地 SQLite 可加 --apply-ddl 建简化表）
robot-dh warehouse init --config configs/warehouse.yaml

# 单日 / 区间 build：自动按 dim → fact → dws → ads 顺序执行
robot-dh warehouse build --date 2026-05-25
robot-dh warehouse build --from-date 2026-05-01 --to-date 2026-05-25 --layers dim,fact,dws,ads

# 通用查询（白名单表 + 简单 WHERE）
robot-dh warehouse query --table ads_quality_dashboard --limit 20 \
                        --where "dt='2026-05-25'" --output table

# 导出 parquet / csv / json + _manifest.json
robot-dh warehouse export --table ads_quality_dashboard --date 2026-05-25 \
                          --format parquet \
                          --output file:///mnt/local-data/robot-dh-local/lake/warehouse/ads
...
```bash
# 日度 summary（top failed rules / p95 / archive log / ml_ready_rows）
robot-dh quality summary --date 2026-05-25 --output json

# 完整 HTML / JSON / CSV / parquet 报告，落 runs/quality_report/
robot-dh quality report --date 2026-05-25 --output runs/quality_report

# SLA：按 configs/sla_policies.yaml 跑校验并写 sla_policies / sla_checks
robot-dh sla check --date 2026-05-25 --policy configs/sla_policies.yaml
robot-dh sla report --date 2026-05-25 --policy configs/sla_policies.yaml --output runs/sla_report

# 补数：从失败 fact_etl_run / fact_workflow_step / sla_checks 派生 backfill plan
robot-dh backfill plan --from-date 2026-05-01 --to-date 2026-05-07 \
                       --dataset droid_lerobot_dev1g --phase normalize \
                       --output runs/backfill
robot-dh backfill status --plan-id <plan_id>
robot-dh backfill run --plan-id <plan_id>           # 默认只打印 recommended commands
robot-dh backfill run --plan-id <plan_id> --execute # 真实 subprocess 执行
```

### 5. 和 v1.7 Argo local workflow 的关系

| v1.7 (运行时) | v1.8 (指标层) |
|---|---|
| `qc_contract_runs` 写一条 | `fact_qc_rule_result` 按 rule 展开 + `dws_rule_failure_daily` 聚合 |
| `workflow_steps` 含 step 级耗时 | `fact_workflow_step` + `dws_workflow_ops_daily` 给出 success_rate / p95 / oom / deadline |
| `asset_profiles` 含 bytes / rows | `fact_asset_profile` 加 `dt` 列；ads 算 raw_bytes / dwd_bytes |
| `ml_ready_datasets` 单纯登记 | `dws_dataset_quality_daily.ml_ready_rows` 算 train+val+test 之和 |
| heartbeat / checkpoint 实时观测 | `stale_heartbeat_count` 进 DWS（运行时 → 离线指标） |

v1.7 是"事件 ingest"层，v1.8 是"离线指标 + 看板"层；两者**只读关系**，v1.8 不会改 v1.7 已有表的写入路径。

### 6. FastAPI 只读端点（v1.8 新增）

```
GET  /warehouse/tables                                 # 15 张表的注册表 + DB 状态
GET  /warehouse/query?table=ads_quality_dashboard&limit=20
GET  /quality/summary?date=2026-05-25                  # 传 date 走 v1.8；无 date 回退 v1.4
GET  /quality/report/latest
GET  /backfill/plans
GET  /backfill/plans/{plan_id}
GET  /sla/checks?date=2026-05-25
```

均为只读；DB 不可达 → 503；不在 API 内执行重 build / backfill run。

### 7. v1.8 端到端命令清单（按执行顺序）

> 目标读者：第一次跑 v1.8 全链路；按顺序复制粘贴即可。所有产出落到 `runs/v1_8/` 一个根目录下，跑完想清理直接 `rm -rf runs/v1_8/`。
...
```bash
export V1_8_DATE=2026-05-25
```

#### 7.1 表存在性检查（warehouse init）

```bash
robot-dh warehouse init --config configs/warehouse.yaml
# 输出 JSON 字段：backend=postgresql / sqlite, existing_tables=[...], missing_tables=[...]
# missing_tables 必须为 []（远端由 infra/migrations/006_v1_8_warehouse_quality_ops.sql 保证）
```

| backend | 行为 |
|---|---|
| `postgresql` | 只校验 14 张表是否存在，**不**自动建表（DDL 由 infra 管） |
| `sqlite` | 自动建表（`ensure_lake_tables`），并在第一次跑时初始化 ORM schema |

#### 7.2 离线指标 build（dim → fact → dws → ads）

```bash
# 单日
robot-dh warehouse build --config configs/warehouse.yaml --date "$V1_8_DATE"

# 区间（建议追跑一周校准 DWS / ADS）
robot-dh warehouse build \
    --config configs/warehouse.yaml \
    --from-date 2026-05-19 --to-date "$V1_8_DATE" \
    --layers dim,fact,dws,ads
```

产出：`status=ok` + 每层耗时 / upsert 行数；远端 PG 模式下 14 张表会被 UPSERT。

> SQL DDL 真源：[`warehouse/sql/ddl/*.sql`](warehouse/sql/ddl/)；DML 真源：[`warehouse/sql/dml/build_*.sql`](warehouse/sql/dml/)。本地 SQLite 走 ORM Python-side 聚合（无 `ON CONFLICT`、无 `PERCENTILE_CONT`），只是为了 `make test` 不依赖 PG。

#### 7.3 看数据（warehouse query / export）

```bash
# 查 ADS 大屏表 top-N
robot-dh warehouse query \
    --config configs/warehouse.yaml \
    --table ads_quality_dashboard --limit 20 \
    --where "dt='$V1_8_DATE'" --order-by "quality_score desc" \
    --output table

# 导出 4 张 fact 表到本地 parquet（供后续 SparkSQL 用）
mkdir -p runs/v1_8/export
for t in dim_dataset fact_etl_run fact_qc_rule_result fact_workflow_step; do
  robot-dh warehouse export \
      --config configs/warehouse.yaml \
      --table "$t" --date "$V1_8_DATE" --format parquet \
      --output "file://$(pwd)/runs/v1_8/export/$t/dt=$V1_8_DATE"
done
# 产出：runs/v1_8/export/<table>/dt=YYYY-MM-DD/<table>.parquet + _manifest.json
```

#### 7.4 质量日报（quality summary / report）

```bash
# 单行 JSON 概况（适合接 monitoring / Slack 通知）
robot-dh quality summary --date "$V1_8_DATE" --output json

# 完整报告：HTML + JSON + CSV + parquet（落 runs/v1_8/quality_report/）
mkdir -p runs/v1_8/quality_report
robot-dh quality report --date "$V1_8_DATE" --output runs/v1_8/quality_report
# 浏览器打开 runs/v1_8/quality_report/quality_summary_*.html 看大屏
```

#### 7.5 SLA 校验（sla check / report）

```bash
# 远端 PG 模式会写 sla_policies / sla_checks；本地 SQLite 同步写
robot-dh sla check --date "$V1_8_DATE" --policy configs/sla_policies.yaml

# HTML / JSON / CSV 报告
robot-dh sla report --date "$V1_8_DATE" --policy configs/sla_policies.yaml \
    --output runs/v1_8/sla
```

#### 7.6 补数计划（backfill plan / status / run）

```bash
# 先 dry-run 看会派生哪些 task（不写 backfill_plans / backfill_tasks）
robot-dh backfill plan \
    --from-date 2026-05-19 --to-date "$V1_8_DATE" \
    --phase normalize --reason "demo dry-run" \
    --dry-run --output runs/v1_8/backfill

# 真写库（去掉 --dry-run）；产出 plan_id 记下来
PLAN_ID=$(robot-dh backfill plan \
    --from-date 2026-05-19 --to-date "$V1_8_DATE" \
    --phase normalize --reason "weekly catchup" \
    --output runs/v1_8/backfill | python3 -c 'import sys,json; print(json.loads(sys.stdin.read())["plan_id"])')

robot-dh backfill status --plan-id "$PLAN_ID"      # 看 task 状态
robot-dh backfill run    --plan-id "$PLAN_ID"      # 默认只打印 recommended_command，不真跑
# robot-dh backfill run --plan-id "$PLAN_ID" --execute  # 想真跑子进程时加 --execute
```

#### 7.7 promptC（可选）：SparkSQL local mode 离线宽表

> 给 Grafana / Trino / Superset / DuckDB 用的 parquet 离线宽表。**不需要 Hadoop / Hive / Operator**。

```bash
# 1) 装可选依赖（pyspark + JDK 11+）
make spark-install
# 若系统无 JDK：conda install -n base -c conda-forge -y openjdk=17
# 然后让 Spark 找到 JDK（conda 路径示例）：
export JAVA_HOME=$CONDA_PREFIX
export PATH="$JAVA_HOME/bin:$PATH"

# 2) 跑 DWS + ADS（读 7.3 export 的 parquet）
robot-dh spark build-quality-ads \
    --input  "file://$(pwd)/runs/v1_8/export" \
    --output "file://$(pwd)/runs/v1_8/spark_ads" \
    --date "$V1_8_DATE"

# 产出：
#   runs/v1_8/spark_ads/_manifest.json
#   runs/v1_8/spark_ads/dws_dataset_quality_daily/dt=YYYY-MM-DD/part-*.snappy.parquet
#   runs/v1_8/spark_ads/ads_quality_dashboard/dt=YYYY-MM-DD/part-*.snappy.parquet

# 任何 BI 工具（DuckDB / Trino / Superset / Grafana parquet plugin）都能直接读
python3 -c "import pyarrow.parquet as pq; print(pq.read_table('runs/v1_8/spark_ads/ads_quality_dashboard').to_pandas().head())"
```

#### 7.8 FastAPI 只读端点（接 Grafana 直查）
...
# Grafana 直接打这两个端点：
curl -s "http://localhost:8000/warehouse/query?table=ads_quality_dashboard&limit=20" | jq
curl -s "http://localhost:8000/quality/summary?date=$V1_8_DATE" | jq

# 其他只读端点
curl -s "http://localhost:8000/backfill/plans?limit=10" | jq
curl -s "http://localhost:8000/sla/checks?date=$V1_8_DATE" | jq
```

#### 7.9 一键回环（懒人入口）

```bash
make v1-8-warehouse-smoke    # 7.1 → 7.2 → 7.3 → 7.4 → 7.6 → 7.5（本地 SQLite，全 mock）
make spark-test              # 7.7 的 pytest 单测（pyspark 未装时整文件 skip）
make test                    # 全量 v1.8 单测（PostgreSQL 集成测试默认 skip，连上 PG 才跑）
```

### 8. promptC：SparkSQL local mode 离线宽表（可选）

| 维度 | PostgreSQL warehouse build（7.2） | SparkSQL local mode（7.7） |
|---|---|---|
| 用途 | 元数据指标 **在线** 查询、FastAPI 喂数 | parquet **离线** 宽表 / 大屏 / BI |
| 输入 | 上游业务表（v1.4-v1.7） | warehouse export 产出的 parquet |
| 输出 | 远端 PostgreSQL 14 张表 | 本地 parquet + `_manifest.json` |
| 依赖 | psycopg / sqlalchemy（**默认装**） | pyspark + JDK 11+（**可选**） |
| Hadoop / Hive | 不需要 | 不需要 |

文件清单：

- `warehouse/spark/sql/build_dws_dataset_quality_daily.sql` — DWS SparkSQL
- `warehouse/spark/sql/build_ads_quality_dashboard.sql` — ADS SparkSQL（含 `quality_score` / `alert_level`）
- `src/robot_dh/spark_jobs/quality_ads.py` — Python 主入口（CLI 调这个）
- `src/robot_dh/spark_jobs/session.py` — `local[*]` SparkSession 工厂
- `tests/test_spark_quality_ads_optional.py` — pytest（pyspark 未装时整文件 skip）

更详细的安装 / 限制 / 何时用哪一边见 [`warehouse/spark/README.md`](warehouse/spark/README.md)。

### 9. 简历 bullet 示例

- 在已有 K8s/Argo 数据平台基础上**自研 DIM/FACT/DWS/ADS 数仓分层**（14 表、SQL build job、UPSERT 幂等），把运行时事件流沉淀为可查询的离线指标层；
- 实现**质量日报 / TopN 失败规则 / workflow SLA / backfill plan**全链路 CLI 与 FastAPI 只读端点，支持本地 SQLite + 远端 PostgreSQL **两套后端共用同一 ORM 与 builder**；
- 增加**可选 SparkSQL local mode 离线宽表**层，复用 warehouse export 的 parquet，无需 Hadoop / Hive 即可产出 Grafana / Trino / Superset 直读的 DWS / ADS parquet。
